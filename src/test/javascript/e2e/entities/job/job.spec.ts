...
describe('Job e2e test', () => {
  ...
  it('should load Jobs', async () => {
    ...
    expect(await jobComponentsPage.getTitle()).to.eq('gitReposApp.job.home.title');
  });

  it('should load create Job page', async () => {
    await jobComponentsPage.clickOnCreateButton();
    jobUpdatePage = new JobUpdatePage();
    expect(await jobUpdatePage.getPageTitle()).to.eq('gitReposApp.job.home.createOrEditLabel');
  ...
  it('should delete last Job', async () => {
    ...
    expect(await jobDeleteDialog.getDialogTitle()).to.eq('gitReposApp.job.delete.question');
