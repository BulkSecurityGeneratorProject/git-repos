...
describe('Task e2e test', () => {
  ...
  it('should load Tasks', async () => {
    ...
    expect(await taskComponentsPage.getTitle()).to.eq('gitReposApp.task.home.title');
  });

  it('should load create Task page', async () => {
    await taskComponentsPage.clickOnCreateButton();
    taskUpdatePage = new TaskUpdatePage();
    expect(await taskUpdatePage.getPageTitle()).to.eq('gitReposApp.task.home.createOrEditLabel');
  ...
  it('should delete last Task', async () => {
    ...
    expect(await taskDeleteDialog.getDialogTitle()).to.eq('gitReposApp.task.delete.question');
